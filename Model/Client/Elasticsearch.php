...
namespace Labofgood\ElasticsearchCluster\Model\Client;
...
class Elasticsearch implements ClientInterface
{
    ...
    /**
     * Initialize Elasticsearch 7 client with cluster support
     *
     * @param array $options
    ...
    private function buildESConfig(array $options = []): array
    {
        ...
        if (!empty($options['enableAuth']) && (int)$options['enableAuth'] === 1) {
            $authString = sprintf('%s:%s@', $options['username'], $options['password']);
        }

        foreach ($options['cluster_hosts'] as $hostRow) {
            $hosts[] = sprintf(
                '%s://%s%s:%d',
                $hostRow['scheme'],
                $authString,
